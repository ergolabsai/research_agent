...
"""Prompt templates used by core services (orchestrator + agents).

These were previously co-located with the Advisor MCP server. They live in
core because the orchestrator and agents consume them directly; the MCP
server re-exports the registry so the stdio prompt-server interface is
unchanged.
"""
