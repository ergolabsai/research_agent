...
import type { NodeLinkGraph, GraphNode, GraphLink } from "../types";

/**
 * Build a mock NodeLinkGraph from a validation result object.
 * This replicates the backend's paper_graph.py construction:
 *  1. Paper node + step nodes + HAS_STEP edges
 *  2. DEPENDS_ON edges between steps (step N depends on step N-1)
 *  3. Evidence nodes + SUPPORTS edges (synthetic from step evidence_count)
 *  4. Figure nodes + ASSESSES edges
 *  5. Math nodes + ASSESSES edges
 *  6. Related paper nodes + RELATED_TO edges
 */
export function buildMockGraph(result: typeof validationResult): NodeLinkGraph {
  const nodes: GraphNode[] = [];
  const links: GraphLink[] = [];

  const paperId = `paper:${result.paper_id}`;
  ...
  nodes.push({
    ...
    main_claim: result.paper_structure.main_claim,
  });

  // --- Step nodes + HAS_STEP + DEPENDS_ON ---
  for (const step of result.paper_structure.logical_steps) {
    ...
    if (step.step_number > 1) {
      links.push({
        source: stepId,
        target: `step:${step.step_number - 1}`,
  ...
  const evidenceTypes = ["citation", "figure", "math", "observation"];
  const evidenceDescriptions = [
    "Referenced finding supports the methodological approach described in this step.",
    "Figure data corroborates the trend claimed in the argument.",
    "Mathematical derivation underpins the quantitative assertion.",
    "Direct experimental observation consistent with stated hypothesis.",
  ];

  for (const [stepKey, stepVal] of Object.entries(result.step_validations)) {
    const stepNum = parseInt(stepKey, 10);
    for (let i = 0; i < stepVal.evidence_count; i++) {
      const eType = evidenceTypes[i % evidenceTypes.length];
      const evidenceId = `evidence:${eType}:section_${stepKey}:${stepNum}:${i}`;
      nodes.push({
        id: evidenceId,
        node_type: "evidence",
        evidence_type: eType,
        description: evidenceDescriptions[i % evidenceDescriptions.length],
        location: `Section ${stepKey}, paragraph ${i + 1}`,
        supports_step: stepNum,
        excerpt: `"...evidence fragment ${i + 1} from step ${stepKey} analysis..."`,
      });
      links.push({
        source: evidenceId,
        target: `step:${stepNum}`,
  ...
  // --- Figure nodes + ASSESSES edges ---
  for (const [stepKey, stepVal] of Object.entries(result.step_validations)) {
    for (const fig of stepVal.figure_validations) {
      const figId = `figure:${fig.figure_name}`;
      if (!nodes.some((n) => n.id === figId)) {
        nodes.push({
          id: figId,
          node_type: "figure",
          figure_name: fig.figure_name,
          actual_description: `Observed rendering of ${fig.figure_name} showing data patterns and visual elements.`,
          expected_description: `Expected: ${fig.figure_name} should display trends consistent with the claims in Section ${stepKey}.`,
          similarities: fig.validity.confirmations.map((c) => c.slice(0, 80)),
          differences: fig.validity.contradictions.map((c) => c.slice(0, 80)),
        });
      }
      links.push({
        source: figId,
        target: `step:${fig.supports_step}`,
        edge_type: "ASSESSES",
        claim: `${fig.figure_name} assesses step ${fig.supports_step}`,
        confirmations: fig.validity.confirmations,
        contradictions: fig.validity.contradictions,
        location: fig.figure_name,
      });
    }
  }

  // --- Math nodes + ASSESSES edges ---
  for (const [stepKey, stepVal] of Object.entries(result.step_validations)) {
    const stepNum = parseInt(stepKey, 10);
    for (const math of stepVal.math_validations) {
      const mathId = `math:${math.equation_reference}`;
      if (!nodes.some((n) => n.id === mathId)) {
        nodes.push({
          id: mathId,
          node_type: "math",
          equation_reference: math.equation_reference,
          calculation_valid: math.calculation_valid,
          details: math.details,
          formula_used: math.formula_used ?? "",
        });
      }
      links.push({
        source: mathId,
        ...
        edge_type: "ASSESSES",
        calculation_valid: math.calculation_valid,
        location: math.equation_reference,
        description: math.details,
      });
    }
  }

  // --- Related paper nodes + RELATED_TO edges ---
  for (const rp of result.related_papers ?? []) {
    const rpId = `related_paper:${rp.paper_id}`;
    nodes.push({
      id: rpId,
